...
import { getAllConnections } from '../actions/testSuiteListActions';
import {
    getallClassNames,
    SubmitTestCaseData
...
export class EditTestCase extends Component {
    ...
    componentDidMount() {
        ...
        const suite_id =
            this.props.match && this.props.match.params
                ? this.props.match.params.suite_id
                : null;
    ...
    static getDerivedStateFromProps = (nextProps, prevState) => {
        ...
        if (
            prevState.firstLoad &&
            prevState.suiteData !== nextProps.suiteData
        ) {
    ...
    handleChange = (e, index, col_event, reset = false) => {
        switch (col_event) {
            case 2:
                const temp_SuiteData_desc = [
                    ...this.state.CaseData_Description
                ];
                temp_SuiteData_desc[index]['test_description'] = e.target.value;
                this.setState({ suiteData: temp_SuiteData_desc });
                break;
            case 5:
                const temp_SuiteData_table = [
                    ...this.state.CaseData_Description
                ];
                temp_SuiteData_table[index]['src_table'] = reset
                    ? ''
                    : e.target.value;
                this.setState({ suiteData: temp_SuiteData_table });
                break;
            case 6:
                const temp_SuiteData_table_tar = [
                    ...this.state.CaseData_Description
                ];
                temp_SuiteData_table_tar[index]['target_table'] = reset
                    ? ''
                    : e.target.value;
                this.setState({ suiteData: temp_SuiteData_table_tar });
                break;
            case 7:
                const temp_SuiteData_table_col = [
                    ...this.state.CaseData_Description
                ];
            ...
            case 8:
                ...
                temp_SuiteData_table_src_qry[index]['source_query'] = reset
                    ? ''
                    : e.target.value;
            ...
            case 9:
                ...
                temp_SuiteData_table_tar_qry[index]['target_query'] =
                    e.target.value;
    ...
    showClass = (classNameList, classes) => {
        const test = classNameList.map(item => {
            ...
            return (
                <MenuItem value={item[key]['supported_test_class']}>
    ...
    handleDBTypeChange = (index, e) => {
        if (
            e.target.value == 'duplicatecheck' ||
            e.target.value == 'nullcheck'
        ) {
    ...
    renderExistingDBTypes = (ExistingDBlist, classes) => {
        const test = ExistingDBlist.map(item => {
            return (
                <MenuItem value={item.db_connection_id}>
    ...
    showDialog = (index, v_index) => {
        switch (v_index) {
            case 8:
                ...
                ModalData_src['Query'] = this.state.CaseData_Description[
                    index
                ].src_query;
                ...
                this.setState({
                    showQueryModal: true,
                    ModalData: ModalData_src
                });
                break;
            case 9:
                const temp_SuiteData_target = [
                    ...this.state.CaseData_Description
                ];
                let ModalData_tar = temp_SuiteData_target[index];
                ModalData_tar['Query'] = this.state.CaseData_Description[
                    index
                ].target_query;
    ...
    addRow = () => {
        const CaseData_Description_temp = this.state.CaseData_Description.map(
            l => Object.assign({}, l)
        );
    ...
    handleTestSuiteUploadClick = () => {
        ...
        this.state.CaseData_Description.map((item, key) => {
            ...
            temp_obj['source_db_connection_id'] == ''
                        ...
                        <Fragment>
                            {
                                <TableCell className="DropDown-SelectClass">
                                    <Select
                                        className="countCheckSelect"
                                        style={{ width: '10vw' }}
                                        value={eachrow.test_class}
                                        onChange={e =>
                                            this.handleDBTypeChange(index, e)
                                        }
                                    >
                                        {this.showClass(
                            ...
                            }
                            {
                                <TableCell className={classes.tablecell}>
                                    <TextField
                                        autoFocus={true}
                                        className="editSuiteEdit"
                                        placeholder="description"
                                        value={eachrow.test_description}
                                        onChange={() =>
                                            this.handleChange(event, index, 2)
                                        }
                                    />
                                </TableCell>
                            }
                            <TableCell>
                                <Select
                                    ...
                                    disabled={this.CheckNullorDuplicate(
                                        eachrow.test_class
                                    )}
                                    onChange={e =>
                                        this.handleExistingDBTypeChange(
                                            index,
                                            e,
                                            3
                                        )
                                ...
                                >
                                    {this.CheckNullorDuplicate(
                                        eachrow.test_class
                                    )}{' '}
                                    ?{' '}
                                    {this.renderExistingDBTypes(
                                        this.props.allConnections,
                                        classes
                                    )}{' '}
                                    : ""
                                </Select>
                            ...
                            <TableCell className={classes.tablecell}>
                                <TextField
                                    ...
                                    error={ISSPACE.test(
                                        eachrow.src_table.trim()
                                    )}
                                    helperText={
                                        ISSPACE.test(eachrow.src_table.trim())
                                            ? 'Table cannot have space'
                                            : ''
                                    }
                                    onChange={() =>
                                        this.handleChange(event, index, 5)
                                    }
                                />
                            </TableCell>
                            <TableCell className={classes.tablecell}>
                                <TextField
                                    ...
                                    onChange={() =>
                                        this.handleChange(event, index, 6)
                                    }
                                />
                            </TableCell>
                            <TableCell>
                                <TextField
                                    autoFocus={true}
                                    placeholder="column"
                                    value={eachrow.column}
                                    onChange={() =>
                                        this.handleChange(event, index, 7)
                                    }
                                />
                            </TableCell>
                            <TableCell>
                                {eachrow.src_query ? (
                                  ...
                                  <IconButton className = "testSuiteEditIcon">
                                    <BorderColorRoundedIcon
                                        onClick={e => {
                                            this.showDialog(index, 8);
                                        }}
                                      style = {{color: '#69717D'}}
                                    /></IconButton></Tooltip>
                                ) : (
                                  <Tooltip title="Add Source Query" placement="top">
                                    <IconButton
                                        className="testSuiteEditIcon"
                                        disabled={this.CheckNullorDuplicate(
                                            eachrow.test_class
                                        )}
                                    >
                            ...
                            <TableCell>
                                {eachrow.target_query ? (
                                  ...
                                  <IconButton className = "testSuiteEditIcon">
                                    <BorderColorRoundedIcon
                                        onClick={e => {
                                            this.showDialog(index, 9);
                                        }}
                                        style = {{color: '#69717D'}}
                                    /></IconButton></Tooltip>
                                ) : (
                                  <Tooltip title="Add Target Query" placement="top">
                                    <IconButton className="testSuiteEditIcon">
                                        <EditRounded
                                            onClick={e => {
                                                this.showDialog(index, 9);
    ...
    ValidFields = item => {
        if (
            item.test_class == 'nullcheck' ||
            item.test_class == 'duplicatecheck'
        ) {
            return (
                item.test_description &&
                item.target_table &&
                item.target_db_connection_id
            );
        } else {
            return (
                item.test_description &&
    ...
    render() {
        ...
        return (
            <div className="AddSuiteLayout">
                <PlaylistAddIcon className="editsuite" />
                <label className="main_titles">Edit TestCase</label>
                <br />
                <div className="editSuitePaper">
                    <Paper className="paperTestSuite">
                        <div className="editSuitemargin testSuitBodyScroll">
                        ...
                        <Tooltip title="Add Testcase" placement="top">
                            ...
                            <div className="testSuitButtonDiv">
                                ...
                                >
                                    <Button
                                        className="button-create button-colors"
                                        variant="contained"
                                        onClick={() =>
...
const mapStateToProps = state => {
    return {
        ...
        suiteData: state.testSuites.connectionsList
            ? state.testSuites.connectionsList.allCases
            : {},
        allCases: state.testSuites.testCase ? state.testSuites.testCase : {},
        classNameList: state.dbDetailsData.classNameList
            ? state.dbDetailsData.classNameList
            : [],
        allConnections:
            state.testSuites.connectionsList &&
...
const mapDispatchToProps = dispatch => ({
    getTestCaseDetailBySuiteId: (suite_id, bool) =>
        dispatch(getTestCaseDetailBySuiteId(suite_id, bool)),
    getTestCaseByTestCaseId: case_id =>
        dispatch(getTestCaseByTestCaseId(case_id)),
...
export default connect(
    mapStateToProps,
    mapDispatchToProps
)(withStyles(useStyles)(EditTestCase));
